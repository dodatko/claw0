...
Трехслойная луковица повтора оборачивает каждое выполнение агента. Каждый уровень обрабатывает
разный класс отказа:

    Уровень 1 -- Ротация аутентификации: цикл через профили ключей API, пропускайте блокировки.
    Уровень 2 -- Восстановление переполнения: уплотните сообщения при переполнении контекста.
    Уровень 3 -- Цикл использования инструментов: стандартное распределение while True + stop_reason.
...
SYSTEM_PROMPT = (
    ...
    "Используйте инструменты, чтобы помочь пользователю с операциями с файлами и командами оболочки.\n"
    "Будьте лаконичны."
)
...
# ---------------------------------------------------------------------------
# 1. FailoverReason -- классифицировать, почему вызов API не удался
# ---------------------------------------------------------------------------


class FailoverReason(Enum):
    """Каждая причина соответствует разной стратегии повтора."""
...
def classify_failure(exc: Exception) -> FailoverReason:
    """Проверьте строку исключения для определения категории отказа.

    Классификация определяет поведение повтора:
      - overflow  -> уплотнить сообщения и повторить с тем же профилем
      - auth      -> пропустить этот профиль, попробовать следующий
      - rate_limit -> пропустить этот профиль с блокировкой, попробовать следующий
      - timeout   -> короткая блокировка на этом профиле, попробовать следующий
      - billing   -> пропустить этот профиль, попробовать следующий
      - unknown   -> пропустить этот профиль, попробовать следующий
    """
    msg = str(exc).lower()
...
# ---------------------------------------------------------------------------
# 2. AuthProfile -- один ключ API с отслеживанием блокировки
# ---------------------------------------------------------------------------


@dataclass
class AuthProfile:
    """Представляет один ключ API, который может быть включен в обслуживание.

    Поля:
      name            -- удобочитаемая метка
      provider        -- какой поставщик LLM (например "anthropic")
      api_key         -- фактическая строка ключа API
      cooldown_until  -- временная метка unix; пропустить этот профиль до тех пор
      failure_reason  -- последняя причина отказа строка, или None если здоров
      last_good_at    -- временная метка unix последнего успешного вызова
    """
    name: str
    provider: str
    api_key: str
...
# ---------------------------------------------------------------------------
# 3. ProfileManager -- выбрать, отметить и перечислить профили
# ---------------------------------------------------------------------------


class ProfileManager:
    """Управляет пулом AuthProfiles с выбором с учетом блокировки."""

    def __init__(self, profiles: list[AuthProfile]):
        self.profiles = profiles

    def select_profile(self) -> AuthProfile | None:
        """Вернуть первый профиль, для которого истекла блокировка.

        Профили проверяются по порядку. Профиль доступен, если
        time.time() >= cooldown_until. Возвращает None, если все на блокировке.
        """
        now = time.time()
    ...
    def select_all_available(self) -> list[AuthProfile]:
        """Вернуть все профили без блокировки по порядку."""
        now = time.time()
    ...
    def mark_failure(
        ...
        """Поставить профиль на блокировку после отказа.

        Блокировка по умолчанию составляет 5 минут. Ошибки timeout используют более короткую
        блокировку (вызывающий передает cooldown_seconds=60).
        """
        profile.cooldown_until = time.time() + cooldown_seconds
        profile.failure_reason = reason.value
        print_resilience(
            f"Профиль '{profile.name}' -> блокировка {cooldown_seconds:.0f}s "
            f"(причина: {reason.value})"
        )

    def mark_success(self, profile: AuthProfile) -> None:
        """Очистить состояние отказа и записать время последнего успеха."""
        profile.failure_reason = None
        profile.last_good_at = time.time()

    def list_profiles(self) -> list[dict[str, Any]]:
        """Вернуть статус всех профилей для отображения."""
        now = time.time()
        result = []
...
# ---------------------------------------------------------------------------
# 4. Упрощенный ContextGuard (встроенный, из шаблона s03)
# ---------------------------------------------------------------------------
# Обеспечивает оценку токена и уплотнение сообщений. Полный ContextGuard
# находится в s03_sessions.py; здесь нам нужен только путь уплотнения, который
# Уровень 2 луковицы повтора использует при ошибках переполнения.
# ---------------------------------------------------------------------------


class ContextGuard:
    """Легкая защита от переполнения контекста для устойчивого бегуна."""
    ...
    def estimate_tokens(text: str) -> int:
        """Приблизительная оценка: 1 токен на 4 символа."""
        return len(text) // 4
...
# ---------------------------------------------------------------------------
# 9. Агентский цикл + REPL
# ---------------------------------------------------------------------------


def agent_loop() -> None:
    """Основной цикл агента с устойчивым бегуном."""

    api_key = os.getenv("ANTHROPIC_API_KEY", "")
    base_url = os.getenv("ANTHROPIC_BASE_URL") or None

    # Создать профили демонстрации. In production, each would have a different key.
    ...
    # Резервные модели: попробуйте меньшие/дешевые модели, если основные не работают
    fallback_models = [
