"""
Раздел 04: Каналы -- "Один мозг, много ртов"

Канал инкапсулирует различия платформ, так что цикл агента видит только
объединённый InboundMessage. Добавление новой платформы = реализуй receive() +
send(); цикл остаётся неизменным.

    Telegram ----.                          .---- sendMessage API
    Feishu -------+-- InboundMessage ---+---- im/v1/messages
    CLI (stdin) --'    Цикл агента        '---- print(stdout)

Ментальная модель: каждая платформа другая, но они все производят одинаковый
InboundMessage. Один интерфейс, N реализаций.

Как запустить:  cd claw0 && python ru/s04_channels.py

Требуется в .env:
    ANTHROPIC_API_KEY=sk-ant-xxxxx
    MODEL_ID=claude-sonnet-4-20250514
    # Опционально: TELEGRAM_BOT_TOKEN, FEISHU_APP_ID, FEISHU_APP_SECRET
"""
...
SYSTEM_PROMPT = (
    "Ты полезный AI-ассистент подключённый к множеству каналов обмена сообщениями.\n"
    "Ты можешь сохранять и искать заметки, используя предоставленные инструменты.\n"
    "При ответе будь краток и полезен."
)
...
def print_assistant(text: str, ch: str = "cli") -> None:
    ...
    print(f"\n{GREEN}{BOLD}Ассистент:{RESET} {prefix}{text}\n")
...
class InboundMessage:
    """Все каналы нормализуются в это. Цикл агента видит только InboundMessage."""
    text: str
    sender_id: str
    channel: str = ""
    account_id: str = ""
...
class ChannelAccount:
    """Конфигурация по-ботам. Один тип канала может запускать несколько ботов."""
    channel: str
    account_id: str
    token: str = ""
...
class CLIChannel(Channel):
    ...
    def receive(self) -> InboundMessage | None:
        try:
            text = input(f"{CYAN}{BOLD}Вы > {RESET}").strip()
...
# ---------------------------------------------------------------------------
# Сохранение смещения -- две простые функции
# ---------------------------------------------------------------------------
...
class TelegramChannel(Channel):
    ...
    def __init__(self, account: ChannelAccount) -> None:
        if not HAS_HTTPX:
            raise RuntimeError("TelegramChannel требует httpx: pip install httpx")
        ...
        # Простая дедупликация: множество видимых ID обновлений, очищается периодически
        self._seen: set[int] = set()

        # Буфер медиа-группы: group_id -> {ts, entries}
        self._media_buf: dict[str, dict] = {}

        # Буфер объединения текста: (peer, sender) -> {text, msg, ts}
    ...
    def poll(self) -> list[InboundMessage]:
        ...
        for update in result:
            ...
            # Продвинь смещение, чтобы Telegram не переслал эти обновления
            if uid >= self._offset:
                self._offset = uid + 1
                save_offset(self._offset_path, self._offset)

            # Простая дедупликация через множество; очист на 5000 для ограничения памяти
            ...
            # Медиа-группы буферизируются отдельно (несколько обновлений = один альбом)
            ...
            # Буферизируй текст для объединения (Telegram разбивает длинные вставки)
    ...
    # -- Очист буферизованных сообщений --

    def _flush_all(self) -> list[InboundMessage]:
        ready: list[InboundMessage] = []

        # Очист медиа-группы спустя 500ms молчания
        now = time.monotonic()
...
class FeishuChannel(Channel):
    ...
    def __init__(self, account: ChannelAccount) -> None:
        if not HAS_HTTPX:
            raise RuntimeError("FeishuChannel требует httpx: pip install httpx")
        self.account_id = account.account_id
    ...
    def _refresh_token(self) -> str:
        ...
        try:
            ...
            data = resp.json()
            if data.get("code") != 0:
                print(f"  {RED}[feishu] Ошибка токена: {data.get('msg', '?')}{RESET}")
                return ""
            self._tenant_token = data.get("tenant_access_token", "")
            self._token_expires_at = time.time() + data.get("expire", 7200) - 300
            return self._tenant_token
        except Exception as exc:
            print(f"  {RED}[feishu] Ошибка токена: {exc}{RESET}")
            return ""
    ...
    def _parse_content(self, message: dict) -> tuple[str, list]:
        ...
        if msg_type == "image":
            ...
            return "[изображение]", media
        return "", media

    def parse_event(self, payload: dict, token: str = "") -> InboundMessage | None:
        """Парси callback-событие Feishu. Простая проверка токена для верификации."""
        if self._encrypt_key and token and token != self._encrypt_key:
            print(f"  {RED}[feishu] Верификация токена не удалась{RESET}")
            return None
    ...
    def send(self, to: str, text: str, **kwargs: Any) -> bool:
        ...
        try:
            ...
            data = resp.json()
            if data.get("code") != 0:
                print(f"  {RED}[feishu] Отправка: {data.get('msg', '?')}{RESET}")
                return False
            return True
        except Exception as exc:
            print(f"  {RED}[feishu] Отправка: {exc}{RESET}")
            return False
...
def tool_memory_write(content: str) -> str:
    print_tool("memory_write", f"{len(content)} символов")
    try:
        MEMORY_FILE.parent.mkdir(parents=True, exist_ok=True)
        with open(MEMORY_FILE, "a", encoding="utf-8") as f:
            f.write(f"\n- {content}\n")
        return f"Записано в память: {content[:80]}..."
    except Exception as exc:
        return f"Ошибка: {exc}"
...
def tool_memory_search(query: str) -> str:
    ...
    if not MEMORY_FILE.exists():
        return "Файл памяти пуст."
    try:
        lines = MEMORY_FILE.read_text(encoding="utf-8").splitlines()
        matches = [l for l in lines if query.lower() in l.lower()]
        return "\n".join(matches[:20]) if matches else f"Совпадений не найдено для '{query}'."
    except Exception as exc:
        return f"Ошибка: {exc}"

TOOLS = [
    {"name": "memory_write", "description": "Сохрани заметку в долгосрочную память.",
     "input_schema": {"type": "object", "required": ["content"],
                      "properties": {"content": {"type": "string",
                                                  "description": "Текст для запоминания."}}}},
    {"name": "memory_search", "description": "Поиск через сохранённые заметки памяти.",
     "input_schema": {"type": "object", "required": ["query"],
                      "properties": {"query": {"type": "string",
                                               "description": "Ключевое слово поиска."}}}},
]
...
class ChannelManager:
    ...
    def register(self, channel: Channel) -> None:
        ...
        print_channel(f"  [+] Канал зарегистрирован: {channel.name}")
...
# ---------------------------------------------------------------------------
# Поток фонового опроса Telegram
# ---------------------------------------------------------------------------

def telegram_poll_loop(
    tg: TelegramChannel, queue: list, lock: threading.Lock, stop: threading.Event,
) -> None:
    print_channel(f"  [telegram] Опрос начат для {tg.account_id}")
    while not stop.is_set():
        try:
            ...
            print(f"  {RED}[telegram] Ошибка опроса: {exc}{RESET}")
            stop.wait(5.0)
...
def handle_repl_command(cmd: str, mgr: ChannelManager) -> bool:
    ...
    if cmd == "/accounts":
        for acc in mgr.accounts:
            masked = acc.token[:8] + "..." if len(acc.token) > 8 else "(нет)"
...
def agent_loop() -> None:
    ...
    print_info("=" * 60)
    print_info("  claw0  |  Раздел 04: Каналы")
    print_info(f"  Модель: {MODEL_ID}")
    print_info(f"  Каналы: {', '.join(mgr.list_channels())}")
    print_info("  Команды: /channels /accounts /help  |  quit/exit")
    print_info("=" * 60)
    print()
    ...
    while True:
        ...
        # Ввод CLI (неблокирующий когда Telegram активен)
        if tg_channel:
            import select
