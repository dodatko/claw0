...
"Не просто реактивный -- проактивный"

Поток таймера проверяет "должен ли я запуститься?" и ставит работу в ту же
очередь, что и пользовательские сообщения. Взаимное исключение lane даёт приоритет пользовательским сообщениям.

    Основная lane:      Введение пользователя --> lock.acquire() -------> LLM --> Вывод
    Lane Heartbeat:     Тик таймера --> lock.acquire(False) -+
                                                        |
                                   получена? --нет--> пропустить (пользователь имеет приоритет)
                                      |да
                                  запустить агента --> дедублировать --> очередь
    Сервис Cron:   CRON.json --> tick() --> срок? --> запустить_агента --> логировать

Использование:
    cd claw0
    python ru/s07_heartbeat_cron.py

Требуется: ANTHROPIC_API_KEY, MODEL_ID (настроены в .env)
Файлы рабочего пространства: HEARTBEAT.md, SOUL.md, MEMORY.md, CRON.json
"""
...
class SoulSystem:
    ...
    def load(self) -> str:
        ...
        return "Вы полезный AI-помощник."
...
class MemoryStore:
    ...
    def search_memory(self, query: str) -> str:
        ...
        return "\n".join(matches[:10]) if matches else f"Нет воспоминаний соответствующих '{query}'."
...
MEMORY_TOOLS = [
    {"name": "memory_write",
     ...
     "input_schema": {"type": "object", "properties": {
         ...
         "required": ["content"]}},
    {"name": "memory_search",
     "description": "Поиск долгосрочной памяти на релевантную информацию.",
     "input_schema": {"type": "object", "properties": {
...
# ---------------------------------------------------------------------------
# Помощник агента -- однооборотный вызов LLM (используется heartbeat и cron)
# ---------------------------------------------------------------------------

def run_agent_single_turn(prompt: str, system_prompt: str | None = None) -> str:
    """Однооборотный вызов LLM, без инструментов, возвращает простой текст."""
    sys_prompt = system_prompt or "Вы полезный помощник, выполняющий проверку в фоне."
    try:
...
class HeartbeatRunner:
    ...
    def should_run(self) -> tuple[bool, str]:
        """4 проверки предусловий. Блокировка тестируется отдельно в _execute()."""
    ...
    def _parse_response(self, response: str) -> str | None:
        """HEARTBEAT_OK означает нечего рассказывать."""
    ...
    def _execute(self) -> None:
        """Выполнить один ход heartbeat. Неблокирующий acquire блокировки; выдаёт если занято."""
        ...
        try:
            ...
            with self._queue_lock:
                self._output_queue.append(f"[ошибка heartbeat: {exc}]")
        finally:
            self.running = False
    ...
    def trigger(self) -> str:
        """Вручную запустить heartbeat, обходя проверку интервала."""
        acquired = self.lane_lock.acquire(blocking=False)
        if not acquired:
            return "основная lane занята, не могу запустить"
        self.running = True
        try:
            instructions, sys_prompt = self._build_heartbeat_prompt()
            if not instructions:
                return "HEARTBEAT.md пуст"
            response = run_agent_single_turn(instructions, sys_prompt)
            meaningful = self._parse_response(response)
            if meaningful is None:
                return "HEARTBEAT_OK (нечего рассказывать)"
            if meaningful.strip() == self._last_output:
                return "дублирующееся содержимое (пропущено)"
            self._last_output = meaningful.strip()
            with self._queue_lock:
                self._output_queue.append(meaningful)
            return f"запущен, вывод в очереди ({len(meaningful)} символов)"
        except Exception as exc:
            return f"запуск не удался: {exc}"
        finally:
            self.running = False
            self.last_run_at = time.time()
            self.lane_lock.release()
...
# CronJob + CronService
# ---------------------------------------------------------------------------
# Типы расписания: at (один раз) | every (фиксированный интервал) | cron (выражение из 5 полей)
# Автоматически отключается после N последовательных ошибок. Логировать в cron-runs.jsonl
...
class CronService:
    ...
    def load_jobs(self) -> None:
        ...
        try:
            ...
            print(f"{YELLOW}Ошибка загрузки CRON.json: {exc}{RESET}")
            return
        now = time.time()
    ...
    def _compute_next(self, job: CronJob, now: float) -> float:
        """Вычислить следующее время выполнения. Возвращает 0.0 если больше расписания нет."""
    ...
    def tick(self) -> None:
        """Вызывается каждую секунду; проверяет и выполняет задолженные задания."""
        now = time.time()
        remove_ids: list[str] = []
    ...
    def _run_job(self, job: CronJob, now: float) -> None:
        ...
        try:
            if kind == "agent_turn":
                ...
                if not msg:
                    output, status = "[пустое сообщение]", "пропущено"
                else:
                    sys_prompt = (
                        "Вы выполняете запланированную задачу в фоне. Будьте кратки. "
                ...
                output, status, error = f"[неизвестный вид: {kind}]", "ошибка", f"неизвестный вид: {kind}"
        except Exception as exc:
            status, error, output = "ошибка", str(exc), f"[ошибка cron: {exc}]"
        ...
        if status == "ошибка":
            ...
            if job.consecutive_errors >= CRON_AUTO_DISABLE_THRESHOLD:
                job.enabled = False
                msg = (f"Задание '{job.name}' автоматически отключено после "
                       f"{job.consecutive_errors} последовательных ошибок: {error}")
        ...
        try:
            ...
            pass
        if output and status != "пропущено":
            with self._queue_lock:
    ...
    def trigger_job(self, job_id: str) -> str:
        for job in self.jobs:
            if job.id == job_id:
                ...
                return f"'{job.name}' запущено (ошибок={job.consecutive_errors})"
    ...
    def list_jobs(self) -> list[dict[str, Any]]:
        ...
        for j in self.jobs:
            ...
            result.append({
                ...
                "last_run": datetime.fromtimestamp(j.last_run_at).isoformat() if j.last_run_at > 0 else "никогда",
                "next_run": datetime.fromtimestamp(j.next_run_at).isoformat() if j.next_run_at > 0 else "н/д",
...
def print_repl_help() -> None:
    print_info("Команды REPL:")
    print_info("  /heartbeat         -- статус heartbeat")
    print_info("  /trigger           -- запустить heartbeat прямо сейчас")
    print_info("  /cron              -- список cron заданий")
    print_info("  /cron-trigger <id> -- запустить cron задание")
    print_info("  /lanes             -- статус блокировки lane")
    print_info("  /help              -- эта справка")
    print_info("  quit / exit        -- выход")
...
def agent_loop() -> None:
    ...
    print_info("=" * 60)
    print_info("  claw0  |  Раздел 07: Heartbeat и Cron")
    print_info(f"  Модель: {MODEL_ID}")
    print_info(f"  Heartbeat: {'вкл' if hb_st['enabled'] else 'выкл'} ({heartbeat.interval}s)")
    print_info(f"  Cron задания: {len(cron_svc.jobs)}")
    print_info("  /help для команд. quit чтобы выйти.")
    print_info("=" * 60)
    print()
    ...
    while True:
        ...
        if user_input.startswith("/"):
            ...
            if cmd == "/help":
                ...
                if not jobs:
                    print_info("Нет cron заданий.")
                for j in jobs:
                    tag = f"{GREEN}ВКЛ{RESET}" if j["enabled"] else f"{RED}ВЫКЛ{RESET}"
                    err = f" {YELLOW}ошиб:{j['errors']}{RESET}" if j["errors"] else ""
                    nxt = f" в {j['next_in']}s" if j["next_in"] is not None else ""
                    print(f"  [{tag}] {j['id']} - {j['name']}{err}{nxt}")
            elif cmd == "/cron-trigger":
                if not arg:
                    print(f"{YELLOW}Использование: /cron-trigger <job_id>{RESET}")
                else:
                ...
                print(f"{YELLOW}Неизвестная команда: {cmd}. /help для команд.{RESET}")
            continue

        # Разговор пользователя: блокирующий acquire (пользователь всегда выигрывает)
        lane_lock.acquire()
        try:
                ...
                if response.stop_reason == "end_turn":
                    ...
                    for block in response.content:
                        if block.type != "tool_use":
                            continue
                        print_info(f"  [инструмент: {block.name}]")
