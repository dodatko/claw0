...
Замените один threading.Lock из раздела 07 правильной именованной полосой
система. Каждая полоса - это очередь FIFO с настраиваемым max_concurrency. Задачи
встают в очередь как вызывающие элементы, выполняются в выделенных потоках и возвращают результаты через
объекты concurrent.futures.Future.
    ...
    _task_done    _task_done      _task_done
        |             |               |
    _pump()       _pump()         _pump()
    (вывести из очереди      (вывести из очереди        (вывести из очереди
     следующий если       следующий если         следующий если
...
Требует: ANTHROPIC_API_KEY, MODEL_ID (настроено в .env)
Файлы рабочего пространства: SOUL.md, MEMORY.md, HEARTBEAT.md, CRON.json
"""
...
# ---------------------------------------------------------------------------
# LaneQueue -- одна названная полоса FIFO с контролем параллелизма
# ---------------------------------------------------------------------------


class LaneQueue:
    """Названная очередь FIFO, которая выполняет максимум max_concurrency задач параллельно.

    Каждый встающий в очередь вызываемый элемент выполняется в своем потоке. Результаты доставляются через
    concurrent.futures.Future. Счетчик поколения поддерживает восстановление при перезагрузке:
    когда поколение увеличивается, устаревшие задачи завершаются из старого
    поколения не переиспускают очередь.
    """
    ...
    def enqueue(self, fn: Callable[[], Any], generation: int | None = None) -> concurrent.futures.Future:
        """Добавить вызываемый элемент в очередь. Возвращает Future для результата.

        Если поколение None, используется текущее поколение полосы.
        """
    ...
    def _pump(self) -> None:
        """Извлечь задачи из очереди и выполнить их пока active < max_concurrency.

        Должно быть вызвано при удержании self._condition.
        """
    ...
    def _run_task(
        ...
        """Выполнить fn, установить результат future, затем вызвать _task_done."""
        try:
            result = fn()
    ...
    def _task_done(self, gen: int) -> None:
        """Уменьшить счет активных. Переиспустить только если поколение все еще совпадает."""
        with self._condition:
            self._active_count -= 1
    ...
    def wait_for_idle(self, timeout: float | None = None) -> bool:
        """Блокировать до тех пор, пока active_count == 0 и очередь пуста.

        Возвращает True, если достигнуто состояние неактивности, False при timeout.
        """
...
# ---------------------------------------------------------------------------
# CommandQueue -- маршруты работы в именованные полосы
# ---------------------------------------------------------------------------


class CommandQueue:
    """Центральный диспетчер, который маршрутизирует вызываемые элементы в названные LaneQueues.

    Полосы создаются лениво при первом использовании. reset_all() увеличивает все
    счетчики поколений, так что устаревшие задачи из предыдущего жизненного цикла
    не переиспускают очереди.
    """
    ...
    def get_or_create_lane(self, name: str, max_concurrency: int = 1) -> LaneQueue:
        """Получить существующую полосу или создать новую."""
        with self._lock:
    ...
    def enqueue(self, lane_name: str, fn: Callable[[], Any]) -> concurrent.futures.Future:
        """Маршрутизировать вызываемый элемент в указанную полосу. Возвращает Future."""
        lane = self.get_or_create_lane(lane_name)
        return lane.enqueue(fn)

    def reset_all(self) -> dict[str, int]:
        """Увеличить поколение на всех полосах. Используется для восстановления при перезагрузке.

        Возвращает словарь lane_name -> new_generation.
        """
        result: dict[str, int] = {}
        with self._lock:
    ...
    def wait_for_all(self, timeout: float = 10.0) -> bool:
        """Ожидать, пока все полосы неактивны. Возвращает True, если все неактивны."""
...
class SoulSystem:
    ...
    def load(self) -> str:
        ...
        return "Вы полезный AI-ассистент."
...
class MemoryStore:
    ...
    def write_memory(self, content: str) -> str:
        ...
        return f"Память сохранена ({len(content)} символов)"

    def search_memory(self, query: str) -> str:
        text = self.load_evergreen()
        if not text:
            return "Воспоминаний не найдено."
        matches = [l for l in text.split("\n") if query.lower() in l.lower()]
        return "\n".join(matches[:10]) if matches else f"Воспоминаний, соответствующих '{query}', не найдено."


MEMORY_TOOLS = [
    {"name": "memory_write",
     "description": "Сохраните важный факт или предпочтение в долгосрочную память.",
     "input_schema": {"type": "object", "properties": {
         "content": {"type": "string", "description": "Факт или предпочтение для запоминания."}},
         "required": ["content"]}},
    {"name": "memory_search",
     "description": "Поиск в долгосрочной памяти по релевантной информации.",
     "input_schema": {"type": "object", "properties": {
...
# ---------------------------------------------------------------------------
# Помощник агента -- однооборотный вызов LLM (общий для heartbeat и cron)
# ---------------------------------------------------------------------------


def run_agent_single_turn(prompt: str, system_prompt: str | None = None) -> str:
    """Однооборотный вызов LLM, нет инструментов, возвращает обычный текст."""
    sys_prompt = system_prompt or "Вы полезный ассистент, выполняющий проверку происхождения."
    try:
        ...
        return f"[ошибка агента: {exc}]"


# ---------------------------------------------------------------------------
# HeartbeatRunner -- встает в очередь через CommandQueue
# ---------------------------------------------------------------------------


class HeartbeatRunner:
    """Фоновый пульс, который встает в очередь в полосу пульса.

    На каждый такт проверяет предусловия. Если полоса пульса уже имеет
    активную работу, такт пропускается (неблокирующая семантика). Это заменяет
    сырой шаблон Lock.acquire(blocking=False) из раздела 07 на
    проверку, учитывающую полосу.
    """
    ...
    def should_run(self) -> tuple[bool, str]:
        """Проверка предусловий перед попыткой пульса."""
        ...
        if elapsed < self.interval:
            return False, f"интервал не истек ({self.interval - elapsed:.0f}s осталось)"
    ...
    def _build_heartbeat_prompt(self) -> tuple[str, str]:
        ...
        extra = ""
        if mem:
            extra = f"## Известный контекст\n\n{mem}\n\n"
        extra += f"Текущее время: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    ...
    def heartbeat_tick(self) -> None:
        """Один такт пульса. Встать в очередь только если полоса не занята."""
        ...
        def _on_done(f: concurrent.futures.Future) -> None:
            ...
            try:
                ...
                print_lane(LANE_HEARTBEAT, f"вывод встал в очередь ({len(meaningful)} символов)")
            except Exception as exc:
                with self._queue_lock:
                    self._output_queue.append(f"[ошибка пульса: {exc}]")
    ...
    def status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "last_run": datetime.fromtimestamp(self.last_run_at).isoformat() if self.last_run_at > 0 else "никогда",
...
def agent_loop() -> None:
    ...
    print_info("=" * 60)
    print_info("  claw0  |  Раздел 10: Параллелизм")
    ...
    while True:
        ...
        if user_input.startswith("/"):
            ...
            if cmd == "/help":
                ...
                if len(parts) < 3:
                    print(f"{YELLOW}Использование: /enqueue <lane> <message>{RESET}")
                else:
                    lane_name = parts[1]
                    message = parts[2]
                ...
                if len(parts) < 3:
                    print(f"{YELLOW}Использование: /concurrency <lane> <N>{RESET}")
                else:
                    lane_name = parts[1]
                    try:
